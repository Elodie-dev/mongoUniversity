...
export default class MoviesDAO {
  ...
  static async getMoviesByCountry(countries) {
    ...
    */
    // await movies.find({countries : {$in : countries}}).project({title: true, _id: true})
    ...
    try {
      ...
      // wire.
      cursor = await movies.find({countries : {$in : countries}}).project({title: true, _id: true})
    } catch (e) {
  ...
  static async getMovieByID(id) {
    try {
      ...
      */
      ...
      if(String(e).startsWith("MongoError: E11000") ){
        return null
      } 
